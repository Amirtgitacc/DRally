...
function controlLines(weaponsEnabled: boolean): string[] {
  if (isTouchDevice()) {
    return [
      ...
      'BRK slows · HB handbrake · TURBO boosts',
      weaponsEnabled ? 'FIRE and MINE sit by your right thumb' : 'No weapons this career',
      'Pause and mute sit along the top edge',
    ]
  }
  return [
    weaponsEnabled ? 'Arrows/WASD drive · X fire · C mine' : 'Arrows/WASD drive · no weapons this career',
...
export class PrepareRaceScene extends Phaser.Scene {
  ...
  create() {
    ...
    text(this, 1140, 180, ['DRIVER', `${career.profile.driverName} · ${carById(career.carId).name}`, `Condition ${career.damage}% damage`, '', 'LOADOUT', gear.length ? gear.join(' · ') : 'Stock configuration', `Weapons ${career.profile.weaponsEnabled ? 'enabled' : 'disabled'}`, '', 'RIVALS', ...rivals, '', 'CONTROLS', ...controlLines(career.profile.weaponsEnabled)].join('\n'), { size: 'body', color: C.textBody, lineSpacing: 9 })
