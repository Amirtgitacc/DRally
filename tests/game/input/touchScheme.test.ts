...
import { TOUCH_HUD_SCALE, anchorRight } from '../../../src/game/race/hudScale'
...
describe('computeTouchLayout', () => {
  ...
  describe('scaled HUD footprint (touch HUD readability bump)', () => {
    ...
    it('places the driver/cash box top-left', () => {
      expect(HUD_RESERVED[0]).toEqual({ x: 0, y: 0, w: 520, h: 240 })
    })

    it('places the race-position box top-centre', () => {
      expect(HUD_RESERVED[1]).toEqual({ x: 1920 / 2 - 170, y: 0, w: 340, h: 130 })
    })

    it('grows the standings box (top-right) from the screen edge at touch scale', () => {
      const standings = HUD_RESERVED[2]
      ...
      expect(standings.h).toBeCloseTo(306 * TOUCH_HUD_SCALE, 5)
      expect(standings.y).toBe(0)
      // absolute pins at the shipped 1.4 scale: 1472..1920 wide
    ...
    it('places the hull/speed box bottom-centre', () => {
      expect(HUD_RESERVED[3]).toEqual({ x: 1920 / 2 - 200, y: 1080 - 160, w: 400, h: 160 })
    })

    it('keeps pause and mute clear of the top-centre and top-right HUD boxes in both mirror modes', () => {
      for (const mirrored of [false, true]) {
        const layout = computeTouchLayout(mirrored)
        for (const boxIdx of [1, 2] as const) {
          const box = HUD_RESERVED[boxIdx]
          for (const name of ['pause', 'mute'] as const) {
            ...
            const overlaps =
              ...
              c.y - c.r < box.y + box.h &&
              c.y + c.r > box.y
            expect(overlaps, `${name} overlaps HUD box ${boxIdx} (mirrored=${mirrored})`).toBe(false)
          }
        }
      }
    })
